...
vec2 calculateVelocity(vec3 ray_dir, vec3 ray_origin)
{
    ...
    vec4 prev_screen_space = lastViewProj * vec4((primaryRayPayload.payload.hit_t *ray_dir) + ray_origin, 1.0);
...
void main()
{
    ...
    imageStore(historyNormalDepth[0], ivec2(gl_LaunchIDEXT.xy), vec4(abs(primaryRayPayload.payload.hit_normal), 1 - (primaryRayPayload.payload.hit_t / tmax)));
