...
{
	ivec3 sample_coord = ivec3(mod(gl_LaunchIDEXT.x, 128), mod(gl_LaunchIDEXT.y, 128), frame.data.index);
	vec3 tagent_diffuse = ImageLoad(blue_noise[sample_coord.z], sample_coord.xy).rgb * 2.0 - 1.0;;
	vec3 N = normalize(normal);
	vec3 up = abs(N.z) < 0.999 ? vec3(0.0, 0.0, 1.0) : vec3(1.0, 0.0, 0.0);
	vec3 T = normalize(cross(up, N));
	vec3 B = cross(N, T);
	mat3 TBN = mat3(T, B, N);
	vec3 world_space_diffuse = tbn * tagent_diffuse;
