...
layout(binding = 3, set = 0, rgba32f) readonly uniform image2D inputImage;
layout(binding = 4, set = 0, rgba32f) writeonly uniform image2D outputImage;
layout (binding = 5, set = 0) uniform FrameUBO {
    Frame data;
} frame;
...
void blur(ivec2 offsetDirection)
{
    ...
    for (int i = 1; i <= 4; ++i) {
        ...
        {
            color += imageLoad(inputImage, coord + ivec2(0, i)) * weights[i];
        }
        else {
        ...
        if (dot(normalDepthNegative.rgb, normalDepth.rgb) > 0.99 && abs(normalDepthNegative.w -normalDepth.w) < 0.01)
        {
